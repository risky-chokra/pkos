...
| 4.2 | Dynamic driver management (Plug & Play) | 🟩 | **`S15mdev` hook**: busybox `mdev` ko `/proc/sys/kernel/hotplug` par lagata hai + `/etc/mdev.conf` + `hotplug.sh` → device aate hi `modprobe "$MODALIAS"` (NIC aaya aur `PK_DHCP=yes` ho to `pk-net dhcp` bhi). Band karna ho: `pk_mdev=off` | boot marker `### PK: MDEV-OK (hotplug=/bin/mdev) ###`; `pk-tune report` → `hotplug` row; dongle plug karke `dmesg | tail` |
